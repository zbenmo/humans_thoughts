...
In addition to classification of images, there are a lot of other tasks that can be addressed with supervised learning (with texts, time-series, sound, tabular data, graphs, etc.). **Regression** for example is a task in which the system is expected to predict a value. Identifing where in an image there is a cat, for example, can be achieved with a ML multi-regression model that predicts the top/left and bottom/right of the bounding box in terms of pixel offsets.
...
The big advantage of unsupervised learning over supervised learning is that we remove the dependancy on humans labeling the data points and labelling the data points consistency. Instead we search for intrinsic labels. Pieces of information that exist already in the data.
Because we are not relying on annotations, we can work on significantly more data points.

A potential usage of unsupervied learning, can be to learn a language model, from a lot of texts in the relevant language.
For example, given a (variable length) text, we can produce a fixed-sized vector of floats. This is what the unsupervised learning achieved.  
...
One way to optimize for good and "smart" fixed-sized vector of floats, is to encode a given text into such a vector and then to attempt to decode, or to reconstruct the text from the vector. We should optimize the distance between the original text and the decoded text. The distance should be minimized, hopefully none. This idea is called "Auto Encoder".
